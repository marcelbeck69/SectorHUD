...
namespace SectorHUD
{
    public class OverlayRenderer : IDisposable
    {
        ...
        public OverlayRenderer(string fontName, float fontSize, float startX, float startY)
        {
            _fontName = fontName;
        ...
        private void RunWindowLoop()
        {
            ...
            // Screen.PrimaryScreen kann null sein (z.B. bei Diensten/Headless-Prozessen).
            // Sichere Fallback-Werte verwenden.
            int width = Screen.PrimaryScreen?.Bounds.Width ?? SystemInformation.PrimaryMonitorSize.Width;
            int height = Screen.PrimaryScreen?.Bounds.Height ?? SystemInformation.PrimaryMonitorSize.Height;
            ...
            foreach (string rawLine in lines)
            {
                ...
                foreach (Match match in matches)
                {
                    if (match.Index > lastIndex)
                    {
                        ...
                        DrawTextSegment(gfx, plain, _whiteBrush, lineFont, ref currentX);
                    }
                    string hex = match.Groups["hex"].Value;
                    string coloredText = match.Groups["inner"].Value;
                    SolidBrush brush = GetBrushForHex(gfx, hex);
                    DrawTextSegment(gfx, coloredText, brush, lineFont, ref currentX);
                    lastIndex = match.Index + match.Length;
                }
                if (lastIndex < line.Length)
                {
                    string remaining = line.Substring(lastIndex);
                    DrawTextSegment(gfx, remaining, _whiteBrush, lineFont, ref currentX);
                }
                currentY += lineHeight;
            }
        }

        private void DrawTextSegment(Graphics gfx, string text, SolidBrush brush, Font font, ref float x)
        {
            if (string.IsNullOrEmpty(text)) return;
            gfx.DrawText(font, brush, x, _startY + (font.FontSize / 2) /* einfache Y-Position */, text);
